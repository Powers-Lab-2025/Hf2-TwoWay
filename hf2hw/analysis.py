...
def moderate_column_clustering(coms, box, n_columns=30, threshold=0.1, max_iter=100, 
                             with_outliers=False, cull_prob_threshold=0.05, verbose=False, debug=False):
    """
    Moderate clustering with Gaussian weighted centroids.
    Returns: (anchors, labels) where labels[i] = -1 for outliers
    """
    xy_coords = coms[:2].T
    box_xy = box[:2]
    ...
    if verbose:
        print(f"[MODERATE CLUSTERING] Starting with {n_columns} columns")
    
    active_molecules = np.ones(n_molecules, dtype=bool)
    
    for iteration in range(max_iter):
        assignments = np.zeros(n_molecules, dtype=int)
        assignments[:] = -1
        
        for i in np.where(active_molecules)[0]:
            mol_pos = xy_coords[i]
            min_dist = float('inf')
            for j, anchor in enumerate(anchors):
                dist = pbc_distance(mol_pos, anchor, box_xy)
        ...
        for col_idx in range(len(anchors)):
            mol_indices = np.where((assignments == col_idx) & active_molecules)[0]
            ...
            distances = []
            for mol_idx in mol_indices:
                dist = pbc_distance(xy_coords[mol_idx], anchors[col_idx], box_xy)
                distances.append(dist)
            distances = np.array(distances)
            
            if len(distances) > 1:
                loc, scale = halfnorm.fit(distances)
                weights = halfnorm.pdf(distances, loc=loc, scale=scale)
            ...
            for i, mol_idx in enumerate(mol_indices):
                disp = pbc_displacement(xy_coords[mol_idx], ref_pos, box_xy)
                weighted_displacements.append(weights[i] * disp)
            
            weighted_displacements = np.array(weighted_displacements)
            mean_weighted_disp = np.sum(weighted_displacements, axis=0)
            new_pos = ref_pos + mean_weighted_disp
            
            new_anchors[col_idx] = new_pos
            
            if with_outliers and iteration > 2:
                for i, mol_idx in enumerate(mol_indices):
                    if scale > 0:
                        prob = halfnorm.cdf(distances[i], loc=loc, scale=scale)
                        if np.random.random() > prob and prob < cull_prob_threshold:
                            active_molecules[mol_idx] = False
        ...
        for col_idx in range(len(anchors)):
            ...
            if len(mol_indices) > 0:
                ...
                for mol_idx in mol_indices:
                    disp = pbc_displacement(xy_coords[mol_idx], ref_pos, box_xy)
                    weighted_displacements.append(disp)
                weighted_displacements = np.array(weighted_displacements)
                mean_disp = np.mean(weighted_displacements, axis=0)
                new_anchors[col_idx] = ref_pos + mean_disp
        
        for col_idx in range(len(anchors)):
            column_movements[col_idx] = pbc_distance(new_anchors[col_idx], old_anchors[col_idx], box_xy)
        
        anchors = new_anchors
        max_movement = np.max(column_movements)
        
        if verbose and iteration % 10 == 0:
            n_active = np.sum(active_molecules)
            print(f"[MODERATE CLUSTERING] Iteration {iteration}: max movement = {max_movement:.4f}, active molecules = {n_active}")
        
        if max_movement < threshold:
            if verbose:
                print(f"[MODERATE CLUSTERING] Converged after {iteration + 1} iterations")
            break
    
    labels = assignments.copy()
    labels[~active_molecules] = -1
    ...
    if verbose:
        n_outliers = np.sum(labels == -1)
        print(f"[MODERATE CLUSTERING] Found {n_outliers} outliers out of {n_molecules} molecules")
